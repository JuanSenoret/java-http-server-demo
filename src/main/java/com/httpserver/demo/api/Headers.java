...
public class Headers implements Iterable<Header> {
    ...
    public Date getDate(String name) {
        try {
            ...
            return header == null ? null : utility.parseDate(header);
    ...
    public void writeTo(OutputStream out) throws IOException {
        for (int i = 0; i < count; i++) {
            out.write(utility.getBytes(headers[i].getName(), ": ", headers[i].getValue()));
            out.write(utility.CRLF);
        }
        out.write(utility.CRLF); // ends header block
    }
    ...
    public Map<String, String> getParams(String name) {
        ...
        for (String param : utility.split(get(name), ";", -1)) {
            String[] pair = utility.split(param, "=", 2);
            String val = pair.length == 1 ? "" : utility.trimLeft(utility.trimRight(pair[1], '"'), '"');
