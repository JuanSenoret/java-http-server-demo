...
/**
 * The {@code Utility} class encapsulates the common methods and variables used in the other classes.
 */
public class Utility {
    // ADDED BY JUAN SENORET: a Utility Class was created to share common methods and variables
     ...
     * @see Utility#readToken(InputStream, int, String, int)
     */
    ...
    public static void serveFileContent(File file, Request req, Response resp) throws IOException {
        ...
        if (range == null || len == 0) {
            status = getConditionalStatusResponse(req, lastModified, etag);
        } else {
    ...
    public static String createIndex(File dir, String path) {
        ...
        f.format("<!DOCTYPE html>%n" +
                        "<html><head><title>HTTP Sever %s</title></head>%n" +
