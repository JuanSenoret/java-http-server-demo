...
public class Response implements Closeable {
    ...
    public OutputStream getBody() throws IOException {
        ...
        List<String> te = Arrays.asList(utility.splitElements(headers.get("Transfer-Encoding"), true));
        List<String> ce = Arrays.asList(utility.splitElements(headers.get("Content-Encoding"), true));
    ...
    public void sendHeaders(int status) throws IOException {
        ...
        if (!headers.contains("Date"))
            headers.add("Date", utility.formatDate(System.currentTimeMillis()));
        headers.add("Server", "JLHTTP/2.4");
        out.write(utility.getBytes("HTTP/1.1 ", Integer.toString(status), " ", utility.statuses[status]));
        out.write(utility.CRLF);
        headers.writeTo(out);
    ...
    public void sendHeaders(int status, long length, long lastModified,
        ...
        if (!headers.contains("Content-Length") && !headers.contains("Transfer-Encoding")) {
            ...
            List<String> encodings = Arrays.asList(utility.splitElements(accepted, true));
            String compression = encodings.contains("gzip") ? "gzip" :
                    encodings.contains("deflate") ? "deflate" : null;
            if (compression != null && (length < 0 || length > 300) && utility.isCompressible(ct) && modern) {
        ...
        if (lastModified > 0 && !headers.contains("Last-Modified")) // RFC2616#14.29
            headers.add("Last-Modified", utility.formatDate(Math.min(lastModified, System.currentTimeMillis())));
     ...
     * {@link Utility#escapeHTML escaped}) HTML.
     *
     * @param status the response status
     ...
     * {@link Utility#escapeHTML escape} method.
     *
     * @param status the response status
     * @param text the text body (sent as text/html)
     * @throws IOException if an error occurs
    ...
    public void sendError(int status, String text) throws IOException {
        send(status, String.format(
                ...
                status, utility.statuses[status], status, utility.statuses[status], utility.escapeHTML(text)));
    }
    ...
    public void sendBody(InputStream body, long length, long[] range) throws IOException {
        ...
        if (out != null) {
            ...
            utility.transfer(body, out, length);
        }
    }
